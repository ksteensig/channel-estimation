...
def train_model(N = 16, K = 4, L = 16, freq = 2.4e9, training_size = 500000, validation_size = 0.1, learning_rate = 0.001, snr = [5, 30], sort = False):    
    training_labels, training_data = dg.data_initialization(training_size, N, K, L, freq, sort = sort)
    ...
    with open(f"history/RBN_N={N}_K={K}_L={L}", 'wb') as f:
        pickle.dump(m.history, f)


    model.save(f"models/RBN_N={N}_K={K}_L={L}")    
